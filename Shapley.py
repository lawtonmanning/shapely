...
class ShapNN(object):
    ...
    def _initialize(self):
        ...
        if self.warm_start:
            for var in tf.compat.v1.global_variables():
                try:
                    self.sess.run(var)
                except tf.compat.v1.errors.FailedPreconditionError:
    ...
    def _build_model(self, X, y):
        ...
        if self.activation is None:
            activation = lambda x: tf.compat.v1.nn.relu(x)
        self.input_ph = tf.compat.v1.placeholder(dtype=tf.compat.v1.float32, shape=(None,) + X.shape[1:], name='input')
        self.dropout_ph = tf.compat.v1.placeholder_with_default(
            tf.compat.v1.constant(0., dtype=tf.compat.v1.float32), shape=(), name='dropout')
        if self.mode=='regression':
            self.labels = tf.compat.v1.placeholder(dtype=tf.compat.v1.float32, shape=(None, ), name='label')
        else:
            self.labels = tf.compat.v1.placeholder(dtype=tf.compat.v1.int32, shape=(None, ), name='label')
        x = tf.compat.v1.reshape(self.input_ph, shape=(-1, np.prod(X.shape[1:])))
    ...
    def _build_train_op(self):
        ...
        learning_rate = tf.compat.v1.constant(self.learning_rate, tf.compat.v1.float32) ##fixit
        trainable_variables = tf.compat.v1.trainable_variables()
        grads = tf.compat.v1.gradients(self.loss, trainable_variables)
        self.grad_flat = tf.compat.v1.concat([tf.compat.v1.reshape(grad, (-1, 1)) for grad in grads], axis=0)
        ...
        previous_ops = [tf.compat.v1.group(*train_ops)]
        with tf.compat.v1.control_dependencies(previous_ops):
    ...
    def _final_layer(self, x, num_classes, mode):
        ...
        if mode=='regression':
            ...
            self.predictions = tf.compat.v1.reduce_sum(self.logits, axis=-1)
            regression_loss = tf.compat.v1.nn.l2_loss(self.predictions - self.labels) ##FIXIT
            self.prediction_loss = tf.compat.v1.reduce_mean(regression_loss, name='l2')
            residuals = self.predictions - self.labels
            var_predicted = tf.compat.v1.reduce_mean(residuals**2) - tf.compat.v1.reduce_mean(residuals)**2
            var_labels = tf.compat.v1.reduce_mean(self.labels**2) - tf.compat.v1.reduce_mean(self.labels)**2
            ...
            xent_loss = tf.compat.v1.nn.sparse_softmax_cross_entropy_with_logits(
                logits=self.logits, labels=tf.compat.v1.cast(self.labels, tf.compat.v1.int32))
            self.prediction_loss = tf.compat.v1.reduce_mean(xent_loss, name='xent')
            self.predictions = tf.compat.v1.argmax(self.probs, axis=-1, output_type=tf.compat.v1.int32)
            correct_predictions = tf.compat.v1.equal(self.predictions, self.labels)
            self.prediction_score = tf.compat.v1.reduce_mean(tf.compat.v1.cast(correct_predictions, tf.compat.v1.float32))
        self.loss = self.prediction_loss + self._reg_loss()
                
    def _dense(self, x, out_dim, dropout=tf.compat.v1.constant(0.), initializer=None, activation=None):
        ...
        x = tf.compat.v1.nn.dropout(x, 1. - dropout)
        if activation:
            x = activation(x)
    ...
    def _reg_loss(self, order=2):
        ...
        for var in tf.compat.v1.trainable_variables():
            if var.op.name.find(r'DW') > 0 or var.op.name.find(r'CW') > 0: ##FIXIT
                if order==2:
                    losss.append(tf.compat.v1.nn.l2_loss(var))
                elif order==1:
                    losss.append(tf.compat.v1.abs(var))
                else:
                    raise ValueError("Invalid regularization order!")
        return tf.compat.v1.multiply(self.weight_decay, tf.compat.v1.add_n(losss))
...
class CShapNN(ShapNN):
    ...
    def _conv(self, x, filter_size, out_filters, strides, activation=None):
        ...
        x = tf.compat.v1.nn.conv2d(x, kernel, strides, padding='SAME')
        if activation:
            x = activation(x)
        return x
    ...
    def _build_model(self, X, y):
        ...
        if self.mode=='regression':
            ...
            self.labels = tf.compat.v1.placeholder(dtype=tf.compat.v1.int32, shape=(None, ), name='label')
        if len(X.shape[1:]) == 2:
            x = tf.compat.v1.reshape(self.input_ph, [-1, X.shape[0], X.shape[1], 1])
        else:
            x = self.input_ph
        ...
        if self.global_averaging:
            x = tf.compat.v1.reduce_mean(x, axis=(1,2))
        else:
            x = tf.compat.v1.reshape(x, shape=(-1, np.prod(x.get_shape()[1:])))
        for layer, hidden_unit in enumerate(self.hidden_units):
            with tf.compat.v1.variable_scope('dense_{}'.format(layer)):
                x = self._dense(x, hidden_unit, dropout=self.dropout_ph, 
                           initializer=self.initializer, activation=activation)
